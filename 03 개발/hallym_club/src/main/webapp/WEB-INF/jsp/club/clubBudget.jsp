...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/res/clubOld/css/budget.css?after">
<style>
</style>
</head>
<body>
		...
		<div style="text-align: justify;">
			...
			<form method="post" action="clubBudgetAction.do"
				...
				<input type="hidden" name="club_id" value=<%=club_id%>> <input
					type="hidden" name="io_gb_cd" value='<%=io_gb_cd%>'>
						...
						<table id="bg_table">
							...
							<tbody>
								<%
									...
									String total_price = (String) session.getAttribute("totalPrice");
									ArrayList<BudgetVO> budget_list = (ArrayList<BudgetVO>) session.getAttribute("budgetList");
									int i = 1;
								...
								<%=i%>
									.getAttribute('id');
									var tmp = Number(document
											.getElementById(x_id).value);
									document.getElementById(x_id).value = tmp
											.toLocaleString();
								</script>
	...
	<script>
		...
		function trClick(tr) {
			...
			var data = "<tr onclick='javascript:trClick(this);'> <td><input type='checkbox' name='check_idx' value='0' /></td>"
					+ "<td><input type='text' name='use_dt' id='"
					+ dt_id
					+ "' onfocus='this.select();'></td> <td><input type='text' name='contents'></td> <td><input type='text' name='price' id='"
					+ idx_id
					+ "'  onKeyup='"
					+ exp
					+ "; numberWithCommas(this); '"
		...
		$("#del_btn")
				.click(
						function() {
							...
							if (result) {
								$("#bg_table input[name='check_idx']:checked")
								...
								while ($('#bg_table tr').length < 6) {
									...
									var data = "<tr onclick='javascript:trClick(this);'> <td><input type='checkbox' name='check_idx' /></td>"
											+ "<td><input type='text' name='use_dt' id='dt' onfocus='this.select();'></td> <td><input type='text' name='contents'></td> <td><input type='text' name='price' id='in_price'  onKeyup='"
											+ exp
											+ "; numberWithCommas(this);'"
											+ "onfocus='this.select();'></td> </tr>";
								...
								for (var x = 1; x <= $('#bg_table tr').length - 2; x++) {
									...
									$('#bg_table  tr:eq(' + x + ')>td:eq(1)')
											.find('input[type="text"]').attr(
													"id", new_id);
									$('#bg_table  tr:eq(' + x + ')>td:eq(3)')
											.find('input[type="text"]').attr(
													"id", new_id2);

									var sum1 = $('#' + new_id2).val().replace(
											/,/g, '');
									if (sum1 == "") {
										sum1 = "0"
									}
