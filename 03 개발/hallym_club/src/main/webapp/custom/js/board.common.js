    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (typeof data.success != "undefined" && data.success == true) {
          $("ul.2dep-category")
            .siblings("a.b-sel-title")
            .text($m("2차분류선택"));

          $("ul.2dep-category").find("li:not(:first)").remove();

          for (var i = 0; i < data.items.length; i++) {
            $("ul.2dep-category").append(
              '<li><a href="#category" class="searchOption" data-value="' +
                data.items[i].id +
                '" title="' +
                data.items[i].categoryNm +
                '">' +
                data.items[i].categoryNm +
                "</a></li>"
            );
  ...
  return {
    ...
    thumbAltCheck: function () {
      $(".thumb").change(function () {
        var $this = $(this),
          val = $this.val(),
          alt = $this.parent().siblings().children(".file-alt"),
          isImg = /\.(png|bmp|jpg|gif)$/i.test(val);
        ...
        $(this)
          ...
          .click(function () {
            if ($(this).parent().siblings(".board-faq-answer").is(":hidden")) {
              ...
              $(this).parent().siblings(".board-faq-answer").is(":visible")
            ) {
    ...
    selectBoxBind: function () {
      // 셀렉트 박스 초기값 설정
      function init($box) {
        var val = $box.find("input[type=hidden]").val(),
          text;
    ...
    loadCategory2Depth: function () {
      ...
      $("#category1").change(function () {
        ...
        $cate2
          .next("a")
          .find("span.ui-selectBox-label")
          .text($cate2.find("option:eq(0)").text());
    ...
    initAttachButton: function () {
      ...
      self.changeAttachCount($("#attachCntNew2").val());
      $("#attachCntNew2").change(function () {
        self.changeAttachCount($(this).val());
      });
      ...
      //exhibition 유형 사용시 :사용법 및 매뉴얼
      self.changeAttachCount2($("#attachCntNew2").val());
      $("#attachCntNew2").change(function () {
    ...
    clickRecommend: function () {
      $(".recommend").click(function (e) {
        ...
        $.ajax({
          url:
            "/_common/jsp/recommend/set_point.jsp?dc=" + new Date().getTime(),
          data: {
            mode: mode,
    ...
    bindCaptcha: function () {
      ...
      $("#captcha_soundOn").click(function () {
        self.bindAudioCapcha(0);
      }); //음성듣기버튼에 클릭이벤트 등록
      $("#captcha_soundOnKor").click(function () {
        self.bindAudioCapcha(1);
      }); //한글음성듣기 버튼에 클릭이벤트 등록
    },
    ...
    bindAudioCapcha: function (type) {
      ...
      $.ajax({
        ...
        success: function (resp) {
          ...
          var soundUrl =
            "/_common/jsp/captcha/CaptChaAudio.jsp?" +
            kor +
            "rand=" +
            rand +
            "&ans=" +
